		...
		while (true) {
			...
			case UNSIGNED_INTEGER_STATE: {
				...
				if (!current_char.has_value())// 如果当前已经读到了文件尾，则解析已经读到的字符串为整数
					if (ss >> number)//     解析成功则返回无符号整数类型的token，否则返回编译错误
				...
				else if (miniplc0::isdigit(ch))// 如果读到的字符是数字，则存储读到的字符
					ss << ch;
			...
			case IDENTIFIER_STATE: {
				...
				auto ch = current_char.value();
				std::string  Str;
				if (!current_char.has_value()) {// 如果当前已经读到了文件尾，则解析已经读到的字符串
					ss >> Str;
					if (Str== "begin")//如果解析结果是关键字
			...
			case PLUS_SIGN_STATE: {
				...
				return std::make_pair(std::make_optional<Token>(TokenType::PLUS_SIGN, '+', pos, currentPos()), std::optional<CompilationError>());
								...
								// 当前状态为减号的状态
			case MINUS_SIGN_STATE: {// 请填空：回退，并返回减号token
				unreadLast();
